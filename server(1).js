...
// ── ORDERS ────────────────────────────────────────────────────────────────────
app.get('/api/orders', (req, res) => { // Note: open for admin panel; secure with adminMiddleware in production
...
app.post('/api/orders', (req, res) => {
  // Accept both 'customer' and 'customerName' for frontend compatibility
  const customer = req.body.customer || req.body.customerName;
  const { phone, items, address, promoCode, userId } = req.body;
  const method = req.body.method || req.body.paymentMethod || 'UPI';
  ...
  const orderId = `PP-${year}-${seq}`;
  const normalizedMethod = (method || 'UPI').toUpperCase();
  const order = {
    id: orderId,
    orderId,
    userId: userId || null,
    ...
    status: 'received',
    payment: normalizedMethod === 'COD' ? 'pending' : 'paid',
    method: normalizedMethod,
    paymentMethod: normalizedMethod.toLowerCase(),
    address,
